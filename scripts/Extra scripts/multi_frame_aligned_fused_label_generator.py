...
from post_process import process_lidar_data, find_latest_log_file
...
def process_single_frame(frame_index=0):
    """
    Process a single frame from the latest log file
    """
    # Get the latest log file
    log_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../raw_logs'))
    ...
    data = json.loads(lines[frame_index])
    ...
    print(f"Debug - Robot pose: {pose}")
    
    # Get detected objects using post_process function
    detected_objects = process_lidar_data(angles, distances, pose)
    
    return {
        "frame_id": scan.frame_id,
        "objects": detected_objects,
        "robot_pose": pose  # Include robot pose for rotation
    }
...
if __name__ == "__main__":
    ...
    # Create output directory for multi-frame labels
    output_dir = Path("output/multi_frame/labels") / f"Y_{raw_log_name}"
    ...
    # Start from frame 4 (index 4) since that's when we have a full buffer
    # and continue until the last frame
    for idx in tqdm(range(4, len(lines)), desc="Generating multi-frame labels"):
    ...
    print(f"Multi-frame YOLO label files saved to {output_dir}") 