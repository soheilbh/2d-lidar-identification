# Multi-frame scenarios simple fused label generator
# This script processes LiDAR scan data to generate YOLO format labels for object detection
# It uses a buffer system to process multiple frames and generates labels for the middle frame
...
class LabelBuffer:
    ...
    Used to process multiple frames and extract labels from the middle frame.
    """
    def __init__(self, buffer_size=3):
        ...
        Args:
            buffer_size (int): Number of frames to keep in buffer (default: 3)
        """
    ...
    def get_middle_frame(self):
        """
        Get the middle frame from buffer when full.
        This is used for label generation as it represents the temporal center.
        
        Returns:
            dict: Middle frame data or None if buffer not full
        """
        if len(self.frames) < self.buffer_size:
            return None
        return self.frames[1]  # Return middle frame (index 1)
...
if __name__ == "__main__":
    ...
    # Uses 'Y_' prefix to distinguish from RGB files ('X_' prefix)
    output_dir = Path("output/multi_frame/multi_scenarios/simple_fused") / f"Y_3_frame_simple_fused_{raw_log_name}"
    ...
    buffer = LabelBuffer(buffer_size=3)  # Initialize buffer for storing frames
    ...
    with open(latest_log, 'r') as f:
        ...
        for line in pbar:
            ...
            if current_scenario != data.get("scenario_number"):
                buffer = LabelBuffer(buffer_size=3)  # Reset buffer for new scenario to avoid mixing frames
            ...
            # Process and save labels after buffer is full (after 3rd frame)
            if scenario_frame_count >= 2:  # buffer_size - 1 (need 3 frames to start generating)
                middle_frame = buffer.get_middle_frame()
                if middle_frame is not None:
                    # Create LidarScan object from middle frame data
                    scan = LidarScan(middle_frame["raw_scan"], middle_frame["pose"], 
                                   frame_id=middle_frame.get("frame_id", frame_count))
                    angles = scan.angles
                    distances = scan.raw_scan
                    pose = scan.pose
                    robot_theta = pose[2]  # Robot orientation
                    object_details = middle_frame.get('object_details', None)  # Ground truth object information
